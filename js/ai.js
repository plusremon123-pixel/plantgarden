...
;(function () {
  ...
  async function generatePlantData(name, onStatus = () => {}) {
    ...
    const cuttingPreferenceHint = isCuttingPreferredPlant(name, '')
      ? '이 식물은 삽목 정보가 특히 중요하므로 cutting_* 정보를 반드시 채우세요. 파종도 가능하면 파종 상세도 함께 채우세요.'
      : '장미, 수국, 목본성 나무류, 라벤더, 로즈마리, 민트, 구문초, 제라늄처럼 삽목/접목/분주가 일반적인 식물은 cutting_* 정보를 반드시 채우세요.'
...
null 없이 모든 필드를 채워주세요:
{
  ...
  "germination_type": "발아 유형 — 광발아, 암발아, 보통, 정보 없음 중 하나",
  "sowing_cover_depth_mm_min": "씨앗 흙덮기 최소 깊이 mm 정수. 광발아는 0",
  "sowing_cover_depth_mm_max": "씨앗 흙덮기 최대 깊이 mm 정수",
  "germination_days_min": "발아 최소 일수 정수",
  "germination_days_max": "발아 최대 일수 정수",
  "germination_temp_min": "발아 적정 최저 온도 정수",
  "germination_temp_max": "발아 적정 최고 온도 정수",
  "sowing_water_note": "파종 후 물관리 한 문장",
  "sowing_note": "파종 방법 팁 한 문장",
  "cutting_root_days_min": "삽목 발근 최소 일수 정수. 삽목이 일반적이지 않으면 빈 문자열",
  "cutting_root_days_max": "삽목 발근 최대 일수 정수. 삽목이 일반적이지 않으면 빈 문자열",
  "cutting_note": "삽목 관리 팁 한 문장. 삽목이 일반적이지 않으면 빈 문자열",
...
- 앱 운영 필수값은 빈 문자열 금지: 공통 생육/온도/물관리/특징 정보는 반드시 채우세요.
- 꽃·채소·허브·그라스처럼 파종 관리가 일반적인 식물은 sowing/germination/germination_type/흙덮기/발아일/발아온도/파종 물관리/파종 팁을 반드시 채우세요.
- 구문초/제라늄처럼 삽목이 흔해도 발아 가능한 식물은 파종 상세도 반드시 채우세요.
- 파종과 삽목이 둘 다 가능한 식물은 둘 다 채우세요. 삽목 우선이 파종 정보 비움을 의미하지 않습니다.
- 불확실한 필수값은 빈 값 대신 보수적인 일반 범위로 입력하세요.
...
- 장미, 수국, 목본성 나무류처럼 품종 유지가 중요하고 삽목/접목/분주가 일반적인 식물은 cutting_* 정보를 우선 채운다.
  파종으로도 키우는 식물은 sowing/germination 값을 반드시 유지하거나 보강하고, 삽목 정보도 반드시 보강한다.
- water_need, watering_interval_min/max, watering_note는 앱 물주기 알림 필수값이므로 반드시 채운다.
...
규칙: JSON 외 다른 텍스트 없이 순수 JSON만.`
    ...
    return {
      ...
      germination_type: ['광발아','암발아','보통','정보 없음'].includes(dataFinal.germination_type) ? dataFinal.germination_type : null,
      sowing_cover_depth_mm_min: Number.isInteger(Number(dataFinal.sowing_cover_depth_mm_min)) ? Number(dataFinal.sowing_cover_depth_mm_min) : null,
      sowing_cover_depth_mm_max: Number.isInteger(Number(dataFinal.sowing_cover_depth_mm_max)) ? Number(dataFinal.sowing_cover_depth_mm_max) : null,
      germination_days_min: Number.isInteger(Number(dataFinal.germination_days_min)) ? Number(dataFinal.germination_days_min) : null,
      germination_days_max: Number.isInteger(Number(dataFinal.germination_days_max)) ? Number(dataFinal.germination_days_max) : null,
      germination_temp_min: Number.isInteger(Number(dataFinal.germination_temp_min)) ? Number(dataFinal.germination_temp_min) : null,
      germination_temp_max: Number.isInteger(Number(dataFinal.germination_temp_max)) ? Number(dataFinal.germination_temp_max) : null,
      sowing_water_note: dataFinal.sowing_water_note || null,
      sowing_note: dataFinal.sowing_note || null,
      cutting_root_days_min: Number.isInteger(Number(dataFinal.cutting_root_days_min)) ? Number(dataFinal.cutting_root_days_min) : null,
      cutting_root_days_max: Number.isInteger(Number(dataFinal.cutting_root_days_max)) ? Number(dataFinal.cutting_root_days_max) : null,
      cutting_note: dataFinal.cutting_note || null,
  ...
  async function identifyPlantFromImage(base64, mimeType = 'image/jpeg') {
    if (window._supabase?.functions?.invoke) {
      try {
        const { data, error } = await window._supabase.functions.invoke('identify-plant-image', {
          body: { base64, mimeType },
        })
        if (!error && data?.ok) return data.data
        const message = data?.error || error?.message || ''
        if (/groq|api\s*키|GROQ_API_KEY|invalid api key|401/i.test(message)) {
          throw new Error('사진 인식 API 키를 확인해주세요.')
        }
      } catch (err) {
        if (/사진 인식 API 키|GROQ_API_KEY|invalid api key|401/i.test(err?.message ?? '')) throw err
      }
    }
